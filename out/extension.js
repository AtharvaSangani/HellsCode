...
const genericRoasts = [
    ...
    "First thing new devs get: a blindfold and a repo with no comments. If you can't figure it out, you don’t get to commit.",
    "Stop taking error messages personally.",
    "I’d like to think I’m a great mentor... but then I saw your pull request.",
    ...
    "The moment you start copy-pasting ChatGPT answers without understanding them... game over.",
    ...
    "You want more memory? I’ll RAM it right up your stack trace!",
    "Get your functions together!",
    "I debug like a beast. You? You debug like a confused snail.",
    "I create. I code. I cry inside. But I don’t write trash like this.",
    "Three stars? You’d be lucky to get one ⭐ on GitHub.",
    "Your security is in your code. Yours has more holes than Swiss cheese.",
    "Under pressure, you trade perfection. Your bug-filled mess proves that.",
    "I’ve got nothing to hide—unlike your minified code.",
    "Your code smells so bad VS Code installed air freshener.",
    "You say you’re self-taught? Fire the teacher.",
    "You’ve rewritten the same function five times. Want to go for six?",
    "I act on instinct. You act on ChatGPT outputs.",
    "My wife’s a schoolteacher. She has more structure than your entire codebase.",
    "I work like a donkey, not a  prompt engineer.",
    "I’m not a YouTuber. I’m a chef who accidentally wrote 1,000,000 lines of Python.",
    "Families are weird. But this code is weirder.",
    "This industry is full of muppets—and your Git history is proof.",
    "If a future son-in-law asks me to invest in his app: he can f-k off. With a capital F!",
    "I spend more time in my terminal than in my kitchen.",
    "If you want something—earn it. Don’t just fork it.",
    "The busier I got, the fitter I got. Unlike your code, which gets lazier every sprint.",
    "I’ve never met someone I believe in less than you.",
    "Start at the bottom. And by bottom, I mean fixing all your own bugs.",
];
...
async function startHell(editor) {
    ...
    rageLevel = 0;
    // --- Clear any old timers if user runs hell again ---
    if (rageTimer)
        clearTimeout(rageTimer);
    // Roast interval (every 10–12 seconds)
    const roastInterval = setInterval(() => {
        const fullText = editor.document.getText();
        vscode.window.showInformationMessage(generateRoast(fullText), { modal: true });
    }, 17000 + Math.floor(Math.random() * 2000));
    // Function to escalate rage
    const escalateRage = async (delay) => {
        rageTimer = setTimeout(async () => {
            rageLevel++;
            const fullText = editor.document.getText();
            if (rageLevel === 1) {
                // LOW rage
                ...
                escalateRage(30000); // next rage after 20s
            }
            else if (rageLevel === 2) {
                // MEDIUM rage
                editor.edit(editBuilder => {
                    ...
                    editBuilder.replace(new vscode.Range(new vscode.Position(0, 0), new vscode.Position(editor.document.lineCount, 0)), lines.join('\n'));
                });
                vscode.window.showInformationMessage("Gordon is medium pissed 😡 and leaving his mark!", { modal: true });
                escalateRage(60000); // next rage after 50s
            }
            else if (rageLevel >= 3) {
                vscode.window.showErrorMessage("Gordon is extremely angry! Closing VS Code in 10 seconds...", { modal: true });
                await new Promise(resolve => setTimeout(resolve, 10000));
                clearInterval(roastInterval); // stop roasting
                await vscode.commands.executeCommand('workbench.action.closeWindow');
            }
        }, delay);
    };
    // Start the rage meter escalation
    escalateRage(10000); // first rage after 10s
}
// ---------------- ACTIVATE ----------------
